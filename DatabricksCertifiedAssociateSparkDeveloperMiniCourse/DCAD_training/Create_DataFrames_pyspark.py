...
# Update from Create_DataFrames notebook written in Scala
...
itemSchema = "i_item_sk LONG,i_item_id STRING,i_rec_start_date date,i_rec_end_date date,i_item_desc STRING,i_current_price decimal(7,2),i_wholesale_cost decimal(7,2),i_brand_id INT,i_brand STRING,i_class_id INT,i_class STRING,i_category_id INT,i_category STRING,i_manufact_id INT,i_manufact STRING,i_size STRING,i_formulation STRING,i_color STRING,i_units STRING,i_container STRING,i_manager_id INT,i_product_name STRING"

# COMMAND ----------

customerDf = spark.read.format("json").schema(customerDfSchema_DDL).load("/FileStore/tables/dcad_data/customer.json")

# COMMAND ----------

webSalesDf = spark.read.options(**csvFileReadOptions).schema(webSalesSchema).csv("/FileStore/tables/dcad_data/web_sales.csv")

# COMMAND ----------

addressDf = spark.read.parquet("/FileStore/tables/dcad_data/address.parquet")

# COMMAND ----------

itemDf = spark.read.options(**datFileReadOptions).schema(itemSchema).csv("/FileStore/tables/dcad_data/item.dat")
